...
@Directive({
  selector: "[qStack],[qHorizontalStack],[qVerticalStack]",
})
export class StackLayoutDirective extends BaseStyledDirective implements OnChanges {
  ...
  private applyDivider() {
    ...
    for (let i = 0; i < children.length - 1; i++) {
      ...
      if (child.hasAttribute("data-divider")) {
        this.applyQuillarStyles(styles, child as HTMLElement);
        continue;
      }
  ...
  private getDirection() {
    ...
    if (element.hasAttribute("qHorizontalStack")) {
      return "row";
    } else if (element.hasAttribute("qVerticalStack")) {
      return "column";
    }
